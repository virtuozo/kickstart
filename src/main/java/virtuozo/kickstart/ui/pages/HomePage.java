...
public class HomePage implements HomeView {
  private HeroPageLayout layout = new HeroPageLayout();
  ...
  public void render(HasComponents<?, ?> container) {
    ...
    Row row = this.layout.addContainer().addRow();
    Column main = row.addColumn().span(12, ViewPort.SMALL);
    
    main.add(new Paragraph().text("Please, proceed to the following steps:"));
    IconifiedList list = new IconifiedList().attachTo(main);
    ...
    main.add(new Paragraph().text("Steps to run this module"));
    list = new IconifiedList().attachTo(main);
